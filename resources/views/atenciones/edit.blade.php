...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1><i class="fas fa-edit"></i> Editar Atención #{{ $atencion->id_atencion }}</h1>
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('atenciones.update', $atencion->id_atencion) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                    ...
                    <select class="form-select @error('id_docente') is-invalid @enderror" 
                        ...
                        @foreach($docentes as $docente)
                            <option value="{{ $docente->id_docente }}" 
                                {{ old('id_docente', $atencion->id_docente) == $docente->id_docente ? 'selected' : '' }}>
                    ...
                    <select class="form-select @error('id_estudiante') is-invalid @enderror" 
                        ...
                        @foreach($estudiantes as $estudiante)
                            <option value="{{ $estudiante->id_estudiante }}" 
                                {{ old('id_estudiante', $atencion->id_estudiante) == $estudiante->id_estudiante ? 'selected' : '' }}>
                    ...
                    <select class="form-select @error('id_tema') is-invalid @enderror" 
                        ...
                        @foreach($temas as $tema)
                            <option value="{{ $tema->id_tema }}" 
                                {{ old('id_tema', $atencion->id_tema) == $tema->id_tema ? 'selected' : '' }}>
                                {{ $tema->nombre_tema }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" 
                           ...
                           id="semestre" 
                           name="semestre" 
                           value="{{ old('semestre', $atencion->semestre) }}" 
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="time" 
                           ...
                           id="hora_atencion" 
                           name="hora_atencion" 
                           value="{{ old('hora_atencion', substr($atencion->hora_atencion, 0, 5)) }}" 
                           required>
                    @error('hora_atencion')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control @error('consulta_estudiante') is-invalid @enderror" 
                              ...
                              rows="3" 
                              required>{{ old('consulta_estudiante', $atencion->consulta_estudiante) }}</textarea>
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control @error('descripcion_atencion') is-invalid @enderror" 
                              ...
                              rows="3" 
                              required>{{ old('descripcion_atencion', $atencion->descripcion_atencion) }}</textarea>
                ...
                @if($atencion->evidencia)
                    <div class="col-12 mb-3">
                        ...
                        <div class="alert alert-info">
                            <i class="fas fa-file"></i> {{ $atencion->evidencia }}
                            <a href="{{ $atencion->evidencia_url }}" target="_blank" class="btn btn-sm btn-primary ms-2">
                ...
                <!-- Nueva evidencia -->
                <div class="col-12 mb-3">
                    <label for="evidencia" class="form-label">Nueva Evidencia (Opcional)</label>
                    <input type="file" 
                    ...
                    <small class="form-text text-muted">
                        Si sube un nuevo archivo, reemplazará el anterior. Formatos: PDF, Imágenes, Word. Máx 10 MB.
                    </small>
                    @error('evidencia')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
